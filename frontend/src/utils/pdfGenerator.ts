...
export function generateSalesReportPDF(
    ...
    doc.setFontSize(10)
    doc.setTextColor(...secondaryColor)
    doc.text(`Store: ${storeName}`, 20, yPosition)
    yPosition += 6
    doc.text(`Range: ${summary.range.charAt(0).toUpperCase() + summary.range.slice(1)}`, 20, yPosition)
    yPosition += 6
    doc.text(`Date: ${new Date().toLocaleDateString()}`, 20, yPosition)
    ...
    const metrics = [
        { label: 'Gross Sales', value: formatCurrency(summary.grossSales, currency) },
        { label: 'Discounts', value: formatCurrency(summary.discountsTotal, currency) },
        { label: 'Taxes', value: formatCurrency(summary.taxesTotal, currency) },
        { label: 'Net Sales', value: formatCurrency(summary.netSales, currency), bold: true },
        { label: 'Total Orders', value: summary.orderCount.toString() }
    ]
    ...
    metrics.forEach(metric => {
        ...
        doc.text(metric.value, 190, yPosition, { align: 'right' })
        yPosition += 8
    })
    ...
    doc.setTextColor(...secondaryColor)
    doc.text(
        `Generated by SlipSync on ${new Date().toLocaleString()}`,
        105,
        280,
        { align: 'center' }
    )
    ...
    // Download
    const fileName = `sales-report-${summary.range}-${new Date().toISOString().slice(0, 10)}.pdf`
    doc.save(fileName)
}
