...
export function OrderProcessing({
  ...
  const handleProcessOrderWithPdf = async () => {
    ...
    try {
      ...
      generateOrderPDF({
        ...
        total,
        currency,
        customerName: newCustomerForm.name || (selectedCustomerId && selectedCustomerId !== 'walk-in'
          ? customers.find(c => c.id === selectedCustomerId)?.name
          : undefined),
        customerEmail: newCustomerForm.email || (selectedCustomerId && selectedCustomerId !== 'walk-in'
          ? customers.find(c => c.id === selectedCustomerId)?.email ?? undefined
          : undefined),
        customerPhone: newCustomerForm.phone || (selectedCustomerId && selectedCustomerId !== 'walk-in'
          ? customers.find(c => c.id === selectedCustomerId)?.phone ?? undefined
          : undefined),
      ...
      toast.success('Order processed and PDF downloaded successfully!')
                  ...
                  {filteredProducts.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={!hasStock}
                        className={`w-full text-left p-4 hover:bg-accent transition-colors ${isSelected ? 'bg-accent border-l-4 border-l-primary' : ''
              ...
              <div className="rounded-lg border-2 border-dashed border-border bg-muted/20 p-8 text-center">
                ...
                <div className="mt-4 flex justify-center w-full">
                  <Button
                    variant="default"
                    size="sm"
                    onClick={handleProcessOrderWithPdf}
              ...
              <div className="space-y-2">
                {devices.map((device) => (
                  <button
                    ...
                    className={`flex w-full items-center gap-3 rounded-lg border-2 p-3 text-left transition-all ${selectedDevice === device.deviceIdentifier
                      ? 'border-primary bg-primary/5'
                      : 'border-border hover:border-primary/50 hover:bg-accent'
                      }`}
                  >
                    <div
                      className={`rounded-md p-2 ${selectedDevice === device.deviceIdentifier
